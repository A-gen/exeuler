defmodule ExEuler.Problems.Problem0018 do
  @moduledoc """
  # Maximum path sum I
  ...
  ## Answer
  iex> ExEuler.Problems.Problem0018.run
  1074
  """
  ...
  defp solve([previous | [target | tail]]) do
    biggers = previous |> pairs |> pick_biggers
    [Enum.zip(biggers, target) |> Enum.map(&sum/1)]
    |> Enum.concat(tail)
    |> solve
  end

  defp pairs(arr), do: pairs(arr, [])
  defp pairs([], acc), do: acc
  defp pairs([_], acc), do: acc
  defp pairs([a1 | [a2 | _] = t], acc) do
    pairs(t, acc ++ [{a1, a2}])
  end
